...
setConfig() {
    if [ ! -f "$PATH_CONFIG" ]; then
        ...
        echo "SSL_START_PORT=16888" >> $PATH_CONFIG
    fi
...
start() {
    ...
    if [ $? -eq 1 ]; then
        ...
        # getConfig "SSL_START_PORT"
        port=$(getConfig "SSL_START_PORT")
...
installapp() {
    ...
    if [[ ! -f $PATH_CONFIG ]];then
        setConfig SSL_START_PORT $((RANDOM%65535+1))
    fi
...
set_port() {
    ...
    setConfig SSL_START_PORT $choose
